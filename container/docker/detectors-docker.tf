resource "signalfx_detector" "heartbeat" {
  name      = format("%s %s", local.name_prefix, "Docker host heartbeat")
  max_delay = 900

  program_text = <<-EOF
		from signalfx.detectors.not_reporting import not_reporting
		signal = data('cpu.usage.system', filter=filter('plugin', 'docker') and ${local.heartbeat_filters} and ${module.filter-tags.filter_custom})${var.heartbeat_aggregation_function}.publish('signal')
...
resource "signalfx_detector" "cpu" {
  name = format("%s %s", local.name_prefix, "Docker container usage of cpu host")
...
resource "signalfx_detector" "throttling" {
  name = format("%s %s", local.name_prefix, "Docker container cpu throttling time")
...
resource "signalfx_detector" "memory" {
  name = format("%s %s", local.name_prefix, "Docker memory usage")
