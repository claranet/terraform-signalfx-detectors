resource "signalfx_detector" "velero_scheduled_backup_missing" {
  name = format("%s %s", local.name_prefix, "Kubernetes velero successful backup")
...
resource "signalfx_detector" "velero_backup_failure" {
  name = format("%s %s", local.name_prefix, "Kubernetes velero failed backup")
...
resource "signalfx_detector" "velero_backup_partial_failure" {
  name = format("%s %s", local.name_prefix, "Kubernetes velero failed partial backup")
...
resource "signalfx_detector" "velero_backup_deletion_failure" {
  name = format("%s %s", local.name_prefix, "Kubernetes velero failed backup deletion")
...
resource "signalfx_detector" "velero_volume_snapshot_failure" {
  name = format("%s %s", local.name_prefix, "Kubernetes velero failed volume snapshot")
