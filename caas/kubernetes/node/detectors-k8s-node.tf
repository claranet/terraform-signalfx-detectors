resource "signalfx_detector" "heartbeat" {
	...
	program_text = <<-EOF
		...
		signal = data('machine_memory_bytes' and ${module.filter-tags.filter_custom})
...
resource "signalfx_detector" "ready" {
	...
	program_text = <<-EOF
		signal = data('kubernetes.node_ready' and ${module.filter-tags.filter_custom})${var.ready_aggregation_function}.${var.ready_transformation_function}(over='${var.ready_transformation_window}')${var.ready_aggregation_function2}.publish('signal')
...
resource "signalfx_detector" "volume_space" {
	...
	program_text = <<-EOF
		A = data('kubernetes.volume_available_bytes' and ${module.filter-tags.filter_custom})${var.volume_space_aggregation_function}
    B = data('kubernetes.volume_capacity_bytes' and ${module.filter-tags.filter_custom})${var.volume_space_aggregation_function}
