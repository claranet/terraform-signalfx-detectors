resource "signalfx_detector" "nginx_ingress_too_many_5xx" {
	...
	program_text = <<-EOF
		...
		A = data('nginx_ingress_controller_response_size', filter=filter('status', '5*') and ${module.filter-tags.filter_custom})${var.nginx_ingress_too_many_5xx_aggregation_function}
		B = data('nginx_ingress_controller_request_size', filter=filter('status', '5*') and ${module.filter-tags.filter_custom})${var.nginx_ingress_too_many_5xx_aggregation_function}
...
resource "signalfx_detector" "nginx_ingress_too_many_4xx" {
	...
	program_text = <<-EOF
		from signalfx.detectors.aperiodic import aperiodic
		A = data('nginx_ingress_controller_response_size', filter=filter('status', '4*') and ${module.filter-tags.filter_custom})${var.nginx_ingress_too_many_4xx_aggregation_function}
		B = data('nginx_ingress_controller_request_size', filter=filter('status', '4*') and ${module.filter-tags.filter_custom})${var.nginx_ingress_too_many_4xx_aggregation_function}
