resource "signalfx_detector" "nginx_ingress_too_many_5xx" {
	name = "${join("", formatlist("[%s]", var.prefixes))}[${var.environment}] Nginx Ingress 5xx error rate"
	...
	program_text = <<-EOF
		...
		B = data('nginx_ingress_controller_requests' and ${module.filter-tags.filter_custom})${var.nginx_ingress_too_many_5xx_aggregation_function}
		...
		aperiodic.range_detector(signal, ${var.nginx_ingress_too_many_5xx_threshold_warning}, ${var.nginx_ingress_too_many_5xx_threshold_critical}, 'within_range', lasting('${var.nginx_ingress_too_many_5xx_aperiodic_duration}', ${var.nginx_ingress_too_many_5xx_aperiodic_percentage}), upper_strict=${var.nginx_ingress_too_many_5xx_aperiodic_upper_strict}).publish('WARN')
	EOF

	rule {
		description           = "is too high > ${var.nginx_ingress_too_many_5xx_threshold_critical}"
		severity              = "Critical"
		detect_label          = "CRIT"
	...
	rule {
		description           = "is too high > ${var.nginx_ingress_too_many_5xx_threshold_warning}"
		severity              = "Warning"
		detect_label          = "WARN"
...
resource "signalfx_detector" "nginx_ingress_too_many_4xx" {
	name = "${join("", formatlist("[%s]", var.prefixes))}[${var.environment}] Nginx Ingress 4xx error rate"
	...
	program_text = <<-EOF
		...
		B = data('nginx_ingress_controller_requests' and ${module.filter-tags.filter_custom})${var.nginx_ingress_too_many_4xx_aggregation_function}
		...
		aperiodic.range_detector(signal, ${var.nginx_ingress_too_many_4xx_threshold_warning}, ${var.nginx_ingress_too_many_4xx_threshold_critical}, 'within_range', lasting('${var.nginx_ingress_too_many_4xx_aperiodic_duration}', ${var.nginx_ingress_too_many_4xx_aperiodic_percentage}), upper_strict=${var.nginx_ingress_too_many_4xx_aperiodic_upper_strict}).publish('WARN')
	EOF

	rule {
		description           = "is too high > ${var.nginx_ingress_too_many_4xx_threshold_critical}"
		severity              = "Critical"
		detect_label          = "CRIT"
	...
	rule {
		description           = "is too high > ${var.nginx_ingress_too_many_4xx_threshold_warning}"
		severity              = "Warning"
		detect_label          = "WARN"
