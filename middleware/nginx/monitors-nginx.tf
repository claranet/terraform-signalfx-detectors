resource "signalfx_detector" "nginx_heartbeat" {
	...
	program_text = <<-EOF
		...
		signal = data('nginx_requests', filter=(not filter('aws_state', '*terminated}', '*stopped}')) and (not filter('gcp_status', '*TERMINATED}', '*STOPPING}')) and (not filter('azure_power_state', 'stop*', 'deallocat*')) and ${module.filter_custom})
...
resource "signalfx_detector" "nginx_dropped_connections" {
	...
	program_text = <<-EOF
		signal = data('connections.failed', filter=${module.filter_custom})${var.nginx_aggregation_function}.${var.nginx_transformation_function}(over='${var.nginx_transformation_window}')
