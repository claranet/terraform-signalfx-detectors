resource "signalfx_detector" "heartbeat" {
  name      = format("%s %s", local.name_prefix, "RabbitMQ  heartbeat")
  max_delay = 900

  program_text = <<-EOF
    from signalfx.detectors.not_reporting import not_reporting
    signal = data('gauge.node.uptime', filter=${local.heartbeat_filters} and filter('plugin', 'rabbitmq') and ${module.filter-tags.filter_custom})${var.heartbeat_aggregation_function}.publish('signal')
...
resource "signalfx_detector" "file_descriptors" {
  name = format("%s %s", local.name_prefix, "RabbitMQ Node file descriptors usage")
...
resource "signalfx_detector" "processes" {
  name = format("%s %s", local.name_prefix, "RabbitMQ Node process usage")
...
resource "signalfx_detector" "sockets" {
  name = format("%s %s", local.name_prefix, "RabbitMQ Node sockets usage")
...
resource "signalfx_detector" "vm_memory" {
  name = format("%s %s", local.name_prefix, "RabbitMQ Node vm_memory usage")
