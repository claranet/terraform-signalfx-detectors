resource "signalfx_detector" "heartbeat" {
  name      = format("%s %s", local.name_prefix, "PHP-FPM heartbeat")
  max_delay = 900

  program_text = <<-EOF
    from signalfx.detectors.not_reporting import not_reporting
    signal = data('phpfpm_requests.accepted', filter=${local.heartbeat_filters} and ${module.filter-tags.filter_custom})${var.heartbeat_aggregation_function}.publish('signal')
...
resource "signalfx_detector" "php_fpm_connect_idle" {
  name = format("%s %s", local.name_prefix, "PHP-FPM busy workers")
