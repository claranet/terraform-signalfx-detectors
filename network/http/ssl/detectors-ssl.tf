resource "signalfx_detector" "invalid_ssl_certificate" {
	...
	program_text = <<-EOF
		signal = data('http.cert_expiry', filter=filter('isValid', 'false') and ${module.filter-tags.filter_custom})${var.invalid_ssl_certificate_aggregation_function}.count(by=['serverName', 'url']).${var.invalid_ssl_certificate_transformation_function}(over='${var.invalid_ssl_certificate_transformation_window}').publish('signal')
