...
resource "signalfx_detector" "user_errors" {
	...
	program_text = <<-EOF
		...
		aperiodic.range_detector(signal, ${var.user_errors_threshold_warning}, ${var.user_errors_threshold_critical}, 'within_range', lasting('${var.user_errors_aperiodic_duration}', ${var.user_errors_aperiodic_percentage})).publish('WARN')
	EOF
...
resource "signalfx_detector" "server_errors" {
	...
	program_text = <<-EOF
		...
		aperiodic.range_detector(signal, ${var.server_errors_threshold_warning}, ${var.server_errors_threshold_critical}, 'within_range', lasting('${var.server_errors_aperiodic_duration}', ${var.server_errors_aperiodic_percentage})).publish('WARN')
	EOF
