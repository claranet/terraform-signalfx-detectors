...
resource "signalfx_detector" "eventhub_failed_requests" {
  ...
  program_text = <<-EOF
		...
		detect(ON_Condition_CRIT, off=when(signal is None, '${var.eventhub_failed_requests_clear_duration}')).publish('CRIT')
		detect(ON_Condition_WARN, off=when(signal is None, '${var.eventhub_failed_requests_clear_duration}')).publish('WARN')
  EOF
