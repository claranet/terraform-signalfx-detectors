...
resource "signalfx_detector" "eventhub_eventhub_eventhub_errors" {
  ...
  program_text = <<-EOF
		...
		A = data('IncomingRequests', filter=filter('resource_type', 'Microsoft.EventHub/namespaces') and filter('primary_aggregation_type', 'true') and ${module.filter-tags.filter_custom})${var.eventhub_eventhub_eventhub_errors_aggregation_function}
		B = data('SuccessfulRequests', filter=filter('resource_type', 'Microsoft.EventHub/namespaces') and filter('primary_aggregation_type', 'true') and ${module.filter-tags.filter_custom})${var.eventhub_eventhub_eventhub_errors_aggregation_function}
		signal = (((A-B)/A)*100).${var.eventhub_eventhub_eventhub_errors_transformation_function}(over='${var.eventhub_eventhub_eventhub_errors_transformation_window}').publish('signal')
  ...
  rule {
    description           = "is too high > ${var.eventhub_eventhub_eventhub_errors_threshold_critical}"
    severity              = "Critical"
    detect_label          = "CRIT"
    disabled              = coalesce(var.eventhub_eventhub_eventhub_errors_disabled_critical, var.eventhub_eventhub_eventhub_errors_disabled, var.detectors_disabled)
    notifications         = coalescelist(var.eventhub_eventhub_eventhub_errors_notifications_critical, var.eventhub_eventhub_eventhub_errors_notifications, var.notifications)
    parameterized_subject = "[{{ruleSeverity}}]{{{detectorName}}} {{{readableRule}}} ({{inputs.signal.value}}) on {{{dimensions}}}"
  }

  rule {
    description           = "is too high > ${var.eventhub_eventhub_eventhub_errors_threshold_warning}"
    severity              = "Warning"
    detect_label          = "WARN"
    disabled              = coalesce(var.eventhub_eventhub_eventhub_errors_disabled_warning, var.eventhub_eventhub_eventhub_errors_disabled, var.detectors_disabled)
    notifications         = coalescelist(var.eventhub_eventhub_eventhub_errors_notifications_warning, var.eventhub_eventhub_eventhub_errors_notifications, var.notifications)
    parameterized_subject = "[{{ruleSeverity}}]{{{detectorName}}} {{{readableRule}}} ({{inputs.signal.value}}) on {{{dimensions}}}"
  }
}
