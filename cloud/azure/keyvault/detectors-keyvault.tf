resource "signalfx_detector" "heartbeat" {
  ...
  program_text = <<-EOF
		...
		signal = data('ServiceApiHit', filter=filter('resource_type', 'Microsoft.KeyVault/vaults') and filter('primary_aggregation_type', 'true') and (not filter('azure_power_state', 'PowerState/stopping', 'PowerState/stoppped', 'PowerState/deallocating', 'PowerState/deallocated')) and ${module.filter-tags.filter_custom}).publish('signal')
...
resource "signalfx_detector" "api_latency" {
  ...
  program_text = <<-EOF
		signal = data('ServiceApiLatency', filter=filter('resource_type', 'Microsoft.KeyVault/vaults') and not filter('activityname', 'secretlist') and filter('primary_aggregation_type', 'true') and ${module.filter-tags.filter_custom})${var.api_latency_aggregation_function}.${var.api_latency_transformation_function}(over='${var.api_latency_transformation_window}').publish('signal')
