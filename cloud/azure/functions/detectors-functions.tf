resource "signalfx_detector" "heartbeat" {
	...
	program_text = <<-EOF
		...
		signal = data('BytesReceived', filter=filter('resource_type', 'Microsoft.Web/sites') and ('is_Azure_Function', 'true') and ${module.filter-tags.filter_custom}).publish('signal')
...
resource "signalfx_detector" "http_5xx_errors_rate" {
	...
	program_text = <<-EOF
		...
		aperiodic.above_or_below_detector(signal, ${var.http_5xx_errors_rate_threshold_warning}, 'above', lasting('${var.http_5xx_errors_rate_aperiodic_duration}', ${var.http_5xx_errors_rate_aperiodic_percentage})).publish('WARN')
	EOF
...
resource "signalfx_detector" "high_connections_count" {
	...
	program_text = <<-EOF
		from signalfx.detectors.aperiodic import aperiodic
		signal = data('AppConnections', filter=filter('resource_type', 'Microsoft.Web/sites/slots') and ${module.filter-tags.filter_custom})${var.high_connections_count_aggregation_function}.${var.high_connections_count_transformation_function}(over='${var.high_connections_count_transformation_window}').publish('signal')
		...
		aperiodic.above_or_below_detector(signal, ${var.high_connections_count_threshold_warning}, 'above', lasting('${var.high_connections_count_aperiodic_duration}', ${var.high_connections_count_aperiodic_percentage})).publish('WARN')
	EOF
...
resource "signalfx_detector" "high_threads_count" {
	...
	program_text = <<-EOF
		from signalfx.detectors.aperiodic import aperiodic
		signal = data('Threads', filter=filter('resource_type', 'Microsoft.Web/sites/slots') and ${module.filter-tags.filter_custom})${var.high_threads_count_aggregation_function}.${var.high_threads_count_transformation_function}(over='${var.high_threads_count_transformation_window}').publish('signal')
		...
		aperiodic.above_or_below_detector(signal, ${var.high_threads_count_threshold_warning}, 'above', lasting('${var.high_threads_count_aperiodic_duration}', ${var.high_threads_count_aperiodic_percentage})).publish('WARN')
	EOF
