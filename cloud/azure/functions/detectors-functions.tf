resource "signalfx_detector" "heartbeat" {
  name = format("%s %s", local.name_prefix, "Azure Functions heartbeat")
...
resource "signalfx_detector" "http_5xx_errors_rate" {
  name = format("%s %s", local.name_prefix, "Azure Functions HTTP 5xx error rate")
...
resource "signalfx_detector" "high_connections_count" {
  name = format("%s %s", local.name_prefix, "Azure Functions connections count")
...
resource "signalfx_detector" "high_threads_count" {
  name = format("%s %s", local.name_prefix, "Azure Functions thread count")
