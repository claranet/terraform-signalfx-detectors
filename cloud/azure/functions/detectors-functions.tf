resource "signalfx_detector" "heartbeat" {
	...
	program_text = <<-EOF
		...
		signal = data('BytesReceived', filter=filter('resource_type', 'Microsoft.Web/sites') and ('is_Azure_Function', 'true') and filter('primary_aggregation_type', 'true') and ${module.filter-tags.filter_custom}).publish('signal')
...
resource "signalfx_detector" "http_5xx_errors_rate" {
	...
	program_text = <<-EOF
		...
		aperiodic.range_detector(signal, ${var.http_5xx_errors_rate_threshold_warning}, ${var.http_5xx_errors_rate_threshold_critical}, 'within_range', lasting('${var.http_5xx_errors_rate_aperiodic_duration}', ${var.http_5xx_errors_rate_aperiodic_percentage})).publish('WARN')
	EOF
...
resource "signalfx_detector" "high_connections_count" {
	...
	program_text = <<-EOF
		...
		aperiodic.range_detector(signal, ${var.high_connections_count_threshold_warning}, ${var.high_connections_count_threshold_critical}, 'within_range', lasting('${var.high_connections_count_aperiodic_duration}', ${var.high_connections_count_aperiodic_percentage})).publish('WARN')
	EOF
...
resource "signalfx_detector" "high_threads_count" {
	...
	program_text = <<-EOF
		...
		aperiodic.range_detector(signal, ${var.high_threads_count_threshold_warning}, ${var.high_threads_count_threshold_critical}, 'within_range', lasting('${var.high_threads_count_aperiodic_duration}', ${var.high_threads_count_aperiodic_percentage})).publish('WARN')
	EOF
