...
resource "signalfx_detector" "db_4xx_requests" {
  ...
  program_text = <<-EOF
		...
		detect(ON_Condition_CRIT, off=when(signal is None, '${var.db_4xx_requests_clear_duration}') or not ON_Condition_CRIT, mode='split').publish('CRIT')
		detect(ON_Condition_WARN, off=when(signal is None, '${var.db_4xx_requests_clear_duration}') or not ON_Condition_WARN, mode='split').publish('WARN')
  EOF
...
resource "signalfx_detector" "db_5xx_requests" {
  ...
  program_text = <<-EOF
		...
		detect(ON_Condition_CRIT, off=when(signal is None, '${var.db_5xx_requests_clear_duration}') or not ON_Condition_CRIT, mode='split').publish('CRIT')
		detect(ON_Condition_WARN, off=when(signal is None, '${var.db_5xx_requests_clear_duration}') or not ON_Condition_WARN, mode='split').publish('WARN')
  EOF
...
resource "signalfx_detector" "scaling" {
  ...
  program_text = <<-EOF
		...
		detect(ON_Condition_CRIT, off=when(signal is None, '${var.scaling_clear_duration}') or not ON_Condition_CRIT, mode='split').publish('CRIT')
		detect(ON_Condition_WARN, off=when(signal is None, '${var.scaling_clear_duration}') or not ON_Condition_WARN, mode='split').publish('WARN')
  EOF
