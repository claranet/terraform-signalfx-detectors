resource "signalfx_detector" "heartbeat" {
	...
	program_text = <<-EOF
		...
		signal = data('SearchQueriesPerSecond', filter=filter('resource_type', 'Microsoft.Search/searchServices') and ${module.filter-tags.filter_custom}).publish('signal')
...
resource "signalfx_detector" "search_latency" {
	...
	program_text = <<-EOF
		signal = data('SearchLatency', filter=filter('resource_type', 'Microsoft.Search/searchServices') and ${module.filter-tags.filter_custom})${var.search_latency_aggregation_function}.${var.search_latency_transformation_function}(over='${var.search_latency_transformation_window}').publish('signal')
