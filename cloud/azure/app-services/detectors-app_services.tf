resource "signalfx_detector" "heartbeat" {
	...
	program_text = <<-EOF
		...
		signal = data('Requests', filter=filter('resource_type', 'Microsoft.Web/sites') and filter('is_Azure_Function', 'false') and filter('primary_aggregation_type', 'true') and (not filter('azure_power_state', 'PowerState/stopping', 'PowerState/stoppped', 'PowerState/deallocating', 'PowerState/deallocated') and ${module.filter-tags.filter_custom}).publish('signal')
...
resource "signalfx_detector" "response_time" {
	...
	program_text = <<-EOF
		...
		detect(ON_Condition_CRIT, off=when(signal is None, '${var.response_time_clear_duration}')).publish('CRIT')
		detect(ON_Condition_WARN, off=when(signal is None, '${var.response_time_clear_duration}')).publish('WARN')
	EOF
...
resource "signalfx_detector" "http_5xx_errors_count" {
	...
	program_text = <<-EOF
		...
		detect(ON_Condition_CRIT, off=when(signal is None, '${var.http_5xx_errors_count_clear_duration}')).publish('CRIT')
		detect(ON_Condition_WARN, off=when(signal is None, '${var.http_5xx_errors_count_clear_duration}')).publish('WARN')
	EOF
...
resource "signalfx_detector" "http_4xx_errors_count" {
	...
	program_text = <<-EOF
		...
		detect(ON_Condition_CRIT, off=when(signal is None, '${var.http_4xx_errors_count_clear_duration}')).publish('CRIT')
		detect(ON_Condition_WARN, off=when(signal is None, '${var.http_4xx_errors_count_clear_duration}')).publish('WARN')
	EOF
...
resource "signalfx_detector" "http_success_status_rate" {
	...
	program_text = <<-EOF
		...
		detect(ON_Condition_CRIT, off=when(signal is None, '${var.http_success_status_rate_clear_duration}')).publish('CRIT')
		detect(ON_Condition_WARN, off=when(signal is None, '${var.http_success_status_rate_clear_duration}')).publish('WARN')
	EOF
