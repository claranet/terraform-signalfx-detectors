...
resource "signalfx_detector" "failed_messages" {
  ...
  program_text = <<-EOF
		...
		detect(ON_Condition_CRIT, off=when(signal is None, '${var.failed_messages_clear_duration}')).publish('CRIT')
		detect(ON_Condition_WARN, off=when(signal is None, '${var.failed_messages_clear_duration}')).publish('WARN')
  EOF
...
resource "signalfx_detector" "unmatched_events" {
  ...
  program_text = <<-EOF
		...
		detect(ON_Condition_CRIT, off=when(signal is None, '${var.unmatched_events_clear_duration}')).publish('CRIT')
		detect(ON_Condition_WARN, off=when(signal is None, '${var.unmatched_events_clear_duration}')).publish('WARN')
  EOF
