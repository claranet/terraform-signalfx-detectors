...
resource "signalfx_detector" "other_requests" {
  ...
  program_text = <<-EOF
		...
		detect(ON_Condition_CRIT, off=when(signal is None, '${var.other_requests_clear_duration}') or not ON_Condition_CRIT, mode='split').publish('CRIT')
		detect(ON_Condition_WARN, off=when(signal is None, '${var.other_requests_clear_duration}') or not ON_Condition_WARN, mode='split').publish('WARN')
  EOF
...
resource "signalfx_detector" "unauthorized_requests" {
  ...
  program_text = <<-EOF
		...
		detect(ON_Condition_CRIT, off=when(signal is None, '${var.unauthorized_requests_clear_duration}') or not ON_Condition_CRIT, mode='split').publish('CRIT')
		detect(ON_Condition_WARN, off=when(signal is None, '${var.unauthorized_requests_clear_duration}') or not ON_Condition_WARN, mode='split').publish('WARN')
  EOF
