resource "signalfx_detector" "heartbeat" {
  name = format("%s %s", local.name_prefix, "AWS RDS heartbeat")
...
resource "signalfx_detector" "cpu_90_15min" {
  name = format("%s %s", local.name_prefix, "AWS RDS instance CPU")
...
resource "signalfx_detector" "free_space_low" {
  name = format("%s %s", local.name_prefix, "AWS RDS instance free space")
...
resource "signalfx_detector" "replica_lag" {
  name = format("%s %s", local.name_prefix, "AWS RDS replica lag")
