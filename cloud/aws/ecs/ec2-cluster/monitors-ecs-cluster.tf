...
resource "signalfx_detector" "memory_reservation" {
	...
	program_text = <<-EOF
		signal = data('MemoryReservation', filter=filter('namespace', 'AWS/ECS') and filter('stat', 'mean') and ${module.filter-tags.filter_custom})${var.memory_reservation_aggregation_function}.${var.memory_reservation_transformation_function}(over='${var.memory_reservation_transformation_window}')
