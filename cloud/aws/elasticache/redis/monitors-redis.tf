resource "signalfx_detector" "cache_hits" {
	...
	program_text = <<-EOF
		A = data('CacheHits', filter=filter('namespace', 'AWS/ElastiCache') and filter('stat', 'mean') and ${module.filter-tags.filter_custom})${var.cache_hits_aggregation_function}
		B = data('CacheMisses', filter=filter('namespace', 'AWS/ElastiCache') and filter('stat', 'mean') and ${module.filter-tags.filter_custom})${var.cache_hits_aggregation_function}
		signal = (A/(A+B)).scale(100).${var.cache_hits_transformation_function}(over='${var.cache_hits_transformation_window}')
		detect(when(signal < ${var.cache_hits_threshold_critical})).publish('CRIT')
		detect(when(signal < ${var.cache_hits_threshold_warning})).publish('WARN')
	EOF
...
resource "signalfx_detector" "cpu_high" {
	...
	program_text = <<-EOF
		signal = data('EngineCPUUtilization', filter=filter('namespace', 'AWS/ElastiCache') and filter('stat', 'mean') and ${module.filter-tags.filter_custom})${var.cpu_high_aggregation_function}.${var.cpu_high_transformation_function}(over='${var.cpu_high_transformation_window}')
...
resource "signalfx_detector" "replication_lag" {
	...
	program_text = <<-EOF
		signal = data('ReplicationLag', filter=filter('namespace', 'AWS/ElastiCache') and filter('stat', 'mean') and ${module.filter-tags.filter_custom})${var.replication_lag_aggregation_function}.${var.replication_lag_transformation_function}(over='${var.replication_lag_transformation_window}')
...
resource "signalfx_detector" "commands" {
	...
	program_text = <<-EOF
		A = data('GetTypeCmds', filter=filter('namespace', 'AWS/ElastiCache') and filter('stat', 'mean') and ${module.filter-tags.filter_custom})${var.commands_aggregation_function}
		B = data('SetTypeCmds', filter=filter('namespace', 'AWS/ElastiCache') and filter('stat', 'mean') and ${module.filter-tags.filter_custom})${var.commands_aggregation_function}
		signal = (A + B).${var.commands_transformation_function}(over='${var.commands_transformation_window}')
		detect(when(signal <= ${var.commands_threshold_critical})).publish('CRIT')
		detect(when(signal <= ${var.commands_threshold_warning})).publish('WARN')
	EOF

	rule {
		description           = "is too low <= ${var.commands_threshold_critical}"
		severity              = "Critical"
		detect_label          = "CRIT"
	...
	rule {
		description           = "is too low <= ${var.commands_threshold_warning}"
		severity              = "Warning"
		detect_label          = "WARN"
