resource "signalfx_detector" "heartbeat" {
	...
	program_text = <<-EOF
		...
		signal = data('CPUUtilization', filter=filter('stat', 'mean') and filter('namespace', 'AWS/ElastiCache') and (not filter('CacheNodeId', '*')) and ${module.filter-tags.filter_custom})
...
resource "signalfx_detector" "evictions_growing" {
	...
	rule {
		description           = "to fast > ${var.evictions_growing_threshold_critical}"
		severity              = "Critical"
		detect_label          = "CRIT"
	...
	rule {
		description           = "to fast > ${var.evictions_growing_threshold_warning}"
		severity              = "Warning"
		detect_label          = "WARN"
