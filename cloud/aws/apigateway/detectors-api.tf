...
resource "signalfx_detector" "latency" {
  ...
  program_text = <<-EOF
		...
		aperiodic.range_detector(signal, ${var.latency_threshold_warning}, ${var.latency_threshold_critical}, 'within_range', lasting('${var.latency_aperiodic_duration}', ${var.latency_aperiodic_percentage})).publish('WARN')
	EOF
