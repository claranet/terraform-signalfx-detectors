...
resource "signalfx_detector" "latency" {
  name = format("%s %s", local.name_prefix, "AWS ApiGateway latency")
...
resource "signalfx_detector" "http_5xx" {
  name = format("%s %s", local.name_prefix, "AWS ApiGateway HTTP 5xx error rate")
...
resource "signalfx_detector" "http_4xx" {
  name = format("%s %s", local.name_prefix, "AWS ApiGateway HTTP 4xx error rate")
