...
resource "signalfx_detector" "free_space" {
  ...
  program_text = <<-EOF
    signal = data('FreeStorageSpace', filter=filter('namespace', 'AWS/ES') and filter('stat', 'lower') and (not filter('NodeId', '*')) and ${module.filter-tags.filter_custom}).sum(by=['NodeId'])${var.free_space_aggregation_function}${var.free_space_transformation_function}.publish('signal')
...
resource "signalfx_detector" "cpu_90_15min" {
  ...
  program_text = <<-EOF
    signal = data('CPUUtilization', filter=filter('namespace', 'AWS/ES') and filter('stat', 'upper') and filter('NodeId', '*') and ${module.filter-tags.filter_custom}).sum(by=['NodeId'])${var.cpu_90_15min_aggregation_function}${var.cpu_90_15min_transformation_function}.publish('signal')
