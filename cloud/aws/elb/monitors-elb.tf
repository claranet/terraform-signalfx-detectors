resource "signalfx_detector" "heartbeat" {
	...
	program_text = <<-EOF
		...
		signal = data('RequestCount', filter=filter('stat', 'sum') and filter('namespace', 'AWS/ELB') and (not filter('AvailabilityZone', '*')) and ${module.filter-tags.filter_custom})
...
resource "signalfx_detector" "too_much_4xx" {
	...
	rule {
		description           = "are too high > ${var.too_much_4xx_threshold_critical}"
		severity              = "Critical"
		detect_label          = "CRIT"
	...
	rule {
		description           = "are too high > ${var.too_much_4xx_threshold_warning}"
		severity              = "Warning"
		detect_label          = "WARN"
...
resource "signalfx_detector" "too_much_5xx" {
	...
	rule {
		description           = "are too high > ${var.too_much_5xx_threshold_critical}"
		severity              = "Critical"
		detect_label          = "CRIT"
	...
	rule {
		description           = "are too high > ${var.too_much_5xx_threshold_warning}"
		severity              = "Warning"
		detect_label          = "WARN"
...
resource "signalfx_detector" "too_much_4xx_backend" {
	...
	rule {
		description           = "are too high > ${var.too_much_4xx_backend_threshold_critical}"
		severity              = "Critical"
		detect_label          = "CRIT"
	...
	rule {
		description           = "are too high > ${var.too_much_4xx_backend_threshold_warning}"
		severity              = "Warning"
		detect_label          = "WARN"
...
resource "signalfx_detector" "too_much_5xx_backend" {
	...
	rule {
		description           = "are too high > ${var.too_much_5xx_backend_threshold_critical}"
		severity              = "Critical"
		detect_label          = "CRIT"
	...
	rule {
		description           = "are too high > ${var.too_much_5xx_backend_threshold_warning}"
		severity              = "Warning"
		detect_label          = "WARN"
