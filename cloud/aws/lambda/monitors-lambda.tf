resource "signalfx_detector" "pct_errors" {
	...
	program_text = <<-EOF
		...
		A = data('Errors', filter=filter('namespace', 'AWS/Lambda') and filter('stat', 'sum') and ${module.filter-tags.filter_custom}).sum()${var.pct_errors_aggregation_function}
		B = data('Invocations', filter=filter('namespace', 'AWS/Lambda') and filter('stat', 'sum') and ${module.filter-tags.filter_custom}).sum()${var.pct_errors_aggregation_function}
...
resource "signalfx_detector" "errors" {
	...
	program_text = <<-EOF
		from signalfx.detectors.aperiodic import aperiodic
		signal = data('Errors', filter=filter('namespace', 'AWS/Lambda') and filter('stat', 'sum') and ${module.filter-tags.filter_custom}).sum()${var.errors_aggregation_function}.${var.errors_transformation_function}(over='${var.errors_transformation_window}')
...
resource "signalfx_detector" "throttles" {
	...
	program_text = <<-EOF
		from signalfx.detectors.aperiodic import aperiodic
		signal = data('Throttles', filter=filter('namespace', 'AWS/Lambda') and filter('stat', 'sum') and ${module.filter-tags.filter_custom}).sum()${var.throttles_aggregation_function}.${var.throttles_transformation_function}(over='${var.throttles_transformation_window}')
...
resource "signalfx_detector" "invocations" {
	...
	program_text = <<-EOF
		from signalfx.detectors.aperiodic import aperiodic
		signal = data('Invocations', filter=filter('namespace', 'AWS/Lambda') and filter('stat', 'sum') and ${module.filter-tags.filter_custom}).sum()${var.invocations_aggregation_function}.${var.invocations_transformation_function}(over='${var.invocations_transformation_window}')
