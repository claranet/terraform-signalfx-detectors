...
resource "signalfx_detector" "storage_usage" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > 80, lasting=%{if var.storage_usage_lasting_duration_major == null}None%{else}'${var.storage_usage_lasting_duration_major}'%{endif}, at_least=${var.storage_usage_at_least_percentage_major})).publish('MAJOR')
    detect(when(signal > 90, lasting=%{if var.storage_usage_lasting_duration_critical == null}None%{else}'${var.storage_usage_lasting_duration_critical}'%{endif}, at_least=${var.storage_usage_at_least_percentage_critical}) and (not when(signal > 80, lasting=%{if var.storage_usage_lasting_duration_major == null}None%{else}'${var.storage_usage_lasting_duration_major}'%{endif}, at_least=${var.storage_usage_at_least_percentage_major}))).publish('CRIT')
EOF

  rule {
    description           = "is too high > 80%"
    severity              = "Major"
    detect_label          = "MAJOR"
