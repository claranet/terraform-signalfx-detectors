...
resource "signalfx_detector" "cpu" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.cpu_threshold_critical}, lasting=${var.cpu_lasting_duration_critical == null ? "None" : format("'%s'", var.cpu_lasting_duration_critical)}, at_least=${var.cpu_at_least_percentage_critical})).publish('CRIT')
    detect(when(signal > ${var.cpu_threshold_major}, lasting=${var.cpu_lasting_duration_major == null ? "None" : format("'%s'", var.cpu_lasting_duration_major)}, at_least=${var.cpu_at_least_percentage_major}) and (not when(signal > ${var.cpu_threshold_critical}, lasting=${var.cpu_lasting_duration_critical == null ? "None" : format("'%s'", var.cpu_lasting_duration_critical)}, at_least=${var.cpu_at_least_percentage_critical}))).publish('MAJOR')
EOF
...
resource "signalfx_detector" "load" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.load_threshold_critical}, lasting=${var.load_lasting_duration_critical == null ? "None" : format("'%s'", var.load_lasting_duration_critical)}, at_least=${var.load_at_least_percentage_critical})).publish('CRIT')
    detect(when(signal > ${var.load_threshold_major}, lasting=${var.load_lasting_duration_major == null ? "None" : format("'%s'", var.load_lasting_duration_major)}, at_least=${var.load_at_least_percentage_major}) and (not when(signal > ${var.load_threshold_critical}, lasting=${var.load_lasting_duration_critical == null ? "None" : format("'%s'", var.load_lasting_duration_critical)}, at_least=${var.load_at_least_percentage_critical}))).publish('MAJOR')
EOF
...
resource "signalfx_detector" "disk_space" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.disk_space_threshold_critical}, lasting=${var.disk_space_lasting_duration_critical == null ? "None" : format("'%s'", var.disk_space_lasting_duration_critical)}, at_least=${var.disk_space_at_least_percentage_critical})).publish('CRIT')
    detect(when(signal > ${var.disk_space_threshold_major}, lasting=${var.disk_space_lasting_duration_major == null ? "None" : format("'%s'", var.disk_space_lasting_duration_major)}, at_least=${var.disk_space_at_least_percentage_major}) and (not when(signal > ${var.disk_space_threshold_critical}, lasting=${var.disk_space_lasting_duration_critical == null ? "None" : format("'%s'", var.disk_space_lasting_duration_critical)}, at_least=${var.disk_space_at_least_percentage_critical}))).publish('MAJOR')
EOF
...
resource "signalfx_detector" "disk_inodes" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.disk_inodes_threshold_critical}, lasting=${var.disk_inodes_lasting_duration_critical == null ? "None" : format("'%s'", var.disk_inodes_lasting_duration_critical)}, at_least=${var.disk_inodes_at_least_percentage_critical})).publish('CRIT')
    detect(when(signal > ${var.disk_inodes_threshold_major}, lasting=${var.disk_inodes_lasting_duration_major == null ? "None" : format("'%s'", var.disk_inodes_lasting_duration_major)}, at_least=${var.disk_inodes_at_least_percentage_major}) and (not when(signal > ${var.disk_inodes_threshold_critical}, lasting=${var.disk_inodes_lasting_duration_critical == null ? "None" : format("'%s'", var.disk_inodes_lasting_duration_critical)}, at_least=${var.disk_inodes_at_least_percentage_critical}))).publish('MAJOR')
EOF
...
resource "signalfx_detector" "memory" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.memory_threshold_critical}, lasting=${var.memory_lasting_duration_critical == null ? "None" : format("'%s'", var.memory_lasting_duration_critical)}, at_least=${var.memory_at_least_percentage_critical})).publish('CRIT')
    detect(when(signal > ${var.memory_threshold_major}, lasting=${var.memory_lasting_duration_major == null ? "None" : format("'%s'", var.memory_lasting_duration_major)}, at_least=${var.memory_at_least_percentage_major}) and (not when(signal > ${var.memory_threshold_critical}, lasting=${var.memory_lasting_duration_critical == null ? "None" : format("'%s'", var.memory_lasting_duration_critical)}, at_least=${var.memory_at_least_percentage_critical}))).publish('MAJOR')
EOF
