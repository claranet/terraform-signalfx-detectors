...
resource "signalfx_detector" "disk_space_running_out" {
  ...
  program_text = <<-EOF
    ...
    signal = data('disk.utilization', filter=${module.filtering.signalflow})${var.disk_space_running_out_aggregation_function}${var.disk_space_running_out_transformation_function}.publish('signal')
    countdown.hours_left_stream_incr_detector(stream=signal, maximum_capacity=${var.disk_space_running_out_maximum_capacity}, lower_threshold=${var.disk_space_running_out_lower_threshold}, fire_lasting=lasting('${var.disk_space_running_out_fire_lasting_time}', ${var.disk_space_running_out_fire_lasting_time_percent}), clear_threshold=${var.disk_space_running_out_clear_hours_remaining}, clear_lasting=lasting('${var.disk_space_running_out_clear_lasting_time}', ${var.disk_space_running_out_clear_lasting_time_percent}), use_double_ewma=${var.disk_space_running_out_use_ewma}).publish('MAJOR')
EOF

  rule {
    description           = "in ${var.disk_space_running_out_lower_threshold}"
    severity              = "Major"
    detect_label          = "MAJOR"
    disabled              = coalesce(var.disk_space_running_out_disabled, var.detectors_disabled)
    notifications         = try(coalescelist(lookup(var.disk_space_running_out_notifications, "major", []), var.notifications.major), null)
    runbook_url           = try(coalesce(var.disk_space_running_out_runbook_url, var.runbook_url), "")
    tip                   = var.disk_space_running_out_tip
    parameterized_subject = var.message_subject == "" ? local.rule_subject : var.message_subject
    parameterized_body    = var.message_body == "" ? local.rule_body : var.message_body
  }

  max_delay = var.disk_space_running_out_max_delay
}
