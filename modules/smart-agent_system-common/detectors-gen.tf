...
resource "signalfx_detector" "disk_inodes" {
  ...
  program_text = <<-EOF
    signal = data('percent_inodes.used', filter=(not filter('fs_type', 'squashfs') and not filter('type', 'squashfs')) and ${module.filtering.signalflow})${var.disk_inodes_aggregation_function}${var.disk_inodes_transformation_function}.publish('signal')
