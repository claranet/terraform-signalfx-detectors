resource "signalfx_detector" "count" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.count_threshold_major} and signal <= ${var.count_threshold_critical}))).publish('MAJOR')
EOF
...
resource "signalfx_detector" "capacity" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.capacity_threshold_major} and signal <= ${var.capacity_threshold_critical}))).publish('MAJOR')
EOF
...
resource "signalfx_detector" "ingress" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.ingress_threshold_major} and signal <= ${var.ingress_threshold_critical}))).publish('MAJOR')
EOF
...
resource "signalfx_detector" "egress" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.egress_threshold_major} and signal <= ${var.egress_threshold_critical}))).publish('MAJOR')
EOF
...
resource "signalfx_detector" "requests_rate" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.requests_rate_threshold_major} and signal <= ${var.requests_rate_threshold_critical}))).publish('MAJOR')
EOF
