resource "signalfx_detector" "hpa_scale_exceeded_capacity" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal >= ${var.hpa_scale_exceeded_capacity_threshold_major}, lasting=${var.hpa_scale_exceeded_capacity_lasting_duration_major == null ? "None" : format("'%s'", var.hpa_scale_exceeded_capacity_lasting_duration_major)}, at_least=${var.hpa_scale_exceeded_capacity_at_least_percentage_major})).publish('MAJOR')
EOF
