resource "signalfx_detector" "node_status" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal == ${var.node_status_threshold_critical}, lasting=${var.node_status_lasting_duration_critical == null ? "None" : format("'%s'", var.node_status_lasting_duration_critical)}, at_least=${var.node_status_at_least_percentage_critical})).publish('CRIT')
    detect(when(signal < ${var.node_status_threshold_minor}, lasting=${var.node_status_lasting_duration_minor == null ? "None" : format("'%s'", var.node_status_lasting_duration_minor)}, at_least=${var.node_status_at_least_percentage_minor})).publish('MINOR')
EOF
...
resource "signalfx_detector" "node_state" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.node_state_threshold_critical}, lasting=${var.node_state_lasting_duration_critical == null ? "None" : format("'%s'", var.node_state_lasting_duration_critical)}, at_least=${var.node_state_at_least_percentage_critical})).publish('CRIT')
EOF
