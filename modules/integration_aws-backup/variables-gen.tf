...
variable "backup_failed_threshold_critical" {
  description = "Critical threshold for backup_failed detector"
  type        = number
  default     = 0
}
...
variable "backup_job_expired_threshold_critical" {
  description = "Critical threshold for backup_job_expired detector"
  type        = number
  default     = 0
}
...
variable "backup_copy_jobs_failed_threshold_critical" {
  description = "Critical threshold for backup_copy_jobs_failed detector"
  type        = number
  default     = 0
}
...
variable "backup_successful_aggregation_function" {
  description = "Aggregation function and group by for backup_successful detector (i.e. \".mean(by=['host'])\")"
  type        = string
  default     = ""
}

variable "backup_successful_transformation_function" {
  description = "Transformation function for backup_successful detector (i.e. \".mean(over='5m')\")"
  type        = string
  default     = ".min(over='23h')"
}

variable "backup_successful_max_delay" {
  description = "Enforce max delay for backup_successful detector (use \"0\" or \"null\" for \"Auto\")"
  type        = number
  default     = null
}

variable "backup_successful_tip" {
  description = "Suggested first course of action or any note useful for incident handling"
  type        = string
  default     = ""
}

variable "backup_successful_runbook_url" {
  description = "URL like SignalFx dashboard or wiki page which can help to troubleshoot the incident cause"
  type        = string
  default     = ""
}
...
variable "backup_successful_disabled" {
  description = "Disable all alerting rules for backup_successful detector"
  type        = bool
  default     = null
}

variable "backup_successful_threshold_critical" {
  description = "Critical threshold for backup_successful detector"
  type        = number
  default     = 0
}

variable "backup_successful_lasting_duration_critical" {
  description = "Minimum duration that conditions must be true before raising alert"
  type        = string
  default     = "1d"
}

variable "backup_successful_at_least_percentage_critical" {
  description = "Percentage of lasting that conditions must be true before raising alert (>= 0.0 and <= 1.0)"
  type        = number
  default     = 0.9
}
...
variable "backup_rp_partial_threshold_minor" {
  description = "Minor threshold for backup_rp_partial detector"
  type        = number
  default     = 0
}
...
variable "backup_rp_expired_threshold_major" {
  description = "Major threshold for backup_rp_expired detector"
  type        = number
  default     = 0
}
