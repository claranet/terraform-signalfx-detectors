resource "signalfx_detector" "workloads_count" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.workloads_count_threshold_minor}, lasting=${var.workloads_count_lasting_duration_minor == null ? "None" : format("'%s'", var.workloads_count_lasting_duration_minor)}, at_least=${var.workloads_count_at_least_percentage_minor})).publish('MINOR')
    detect(when(signal > ${var.workloads_count_threshold_warning}, lasting=${var.workloads_count_lasting_duration_warning == null ? "None" : format("'%s'", var.workloads_count_lasting_duration_warning)}, at_least=${var.workloads_count_at_least_percentage_warning}) and (not when(signal > ${var.workloads_count_threshold_minor}, lasting=${var.workloads_count_lasting_duration_minor == null ? "None" : format("'%s'", var.workloads_count_lasting_duration_minor)}, at_least=${var.workloads_count_at_least_percentage_minor}))).publish('WARN')
EOF
