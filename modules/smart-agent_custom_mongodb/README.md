...
module "signalfx-detectors-smart-agent-mongodb" {
  source = "github.com/claranet/terraform-signalfx-detectors.git//modules/smart-agent_custom_mongodb?ref={revision}"
...
(webchecks often require accuracy). I.e. `max(over='15m')` on `mongodb_code_matched` will
