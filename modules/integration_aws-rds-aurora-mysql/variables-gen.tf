...
variable "aurora_mysql_replica_lag_transformation_function" {
  description = "Transformation function for aurora_mysql_replica_lag detector (i.e. \".mean(over='5m')\")"
  type        = string
  default     = ".min(over='5m')"
}
...
variable "aurora_mysql_replica_lag_lasting_duration_critical" {
  description = "Minimum duration that conditions must be true before raising alert"
  type        = string
  default     = null
}

variable "aurora_mysql_replica_lag_at_least_percentage_critical" {
...
variable "aurora_mysql_replica_lag_lasting_duration_major" {
  description = "Minimum duration that conditions must be true before raising alert"
  type        = string
  default     = null
}

variable "aurora_mysql_replica_lag_at_least_percentage_major" {
  description = "Percentage of lasting that conditions must be true before raising alert (>= 0.0 and <= 1.0)"
  type        = number
  default     = 1
}
