...
resource "signalfx_detector" "out_of_memory_errors" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.out_of_memory_errors_threshold_critical}, lasting=%{if var.out_of_memory_errors_lasting_duration_critical == null}None%{else}'${var.out_of_memory_errors_lasting_duration_critical}'%{endif}, at_least=${var.out_of_memory_errors_at_least_percentage_critical})).publish('CRIT')
EOF
...
resource "signalfx_detector" "disk_write_queue" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.disk_write_queue_threshold_critical}, lasting=%{if var.disk_write_queue_lasting_duration_critical == null}None%{else}'${var.disk_write_queue_lasting_duration_critical}'%{endif}, at_least=${var.disk_write_queue_at_least_percentage_critical})).publish('CRIT')
    detect(when(signal > ${var.disk_write_queue_threshold_major}, lasting=%{if var.disk_write_queue_lasting_duration_major == null}None%{else}'${var.disk_write_queue_lasting_duration_major}'%{endif}, at_least=${var.disk_write_queue_at_least_percentage_major}) and (not when(signal > ${var.disk_write_queue_threshold_critical}, lasting=%{if var.disk_write_queue_lasting_duration_critical == null}None%{else}'${var.disk_write_queue_lasting_duration_critical}'%{endif}, at_least=${var.disk_write_queue_at_least_percentage_critical}))).publish('MAJOR')
EOF
