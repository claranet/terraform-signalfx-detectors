resource "signalfx_detector" "heartbeat" {
  name = format("%s %s", local.detector_name_prefix, "AWS ECS Cluster heartbeat")
  ...
  program_text = <<-EOF
    ...
    signal = data('CPUReservation', filter=base_filtering and filter('stat', 'mean') and ${module.filtering.signalflow})${var.heartbeat_aggregation_function}.publish('signal')
...
resource "signalfx_detector" "cpu_utilization" {
  ...
  program_text = <<-EOF
    base_filtering = filter('namespace', 'AWS/ECS')
    signal = data('CPUUtilization', filter=base_filtering and filter('stat', 'mean') and not filter('ServiceName', '*') and ${module.filtering.signalflow})${var.cpu_utilization_aggregation_function}${var.cpu_utilization_transformation_function}.publish('signal')
...
resource "signalfx_detector" "memory_utilization" {
  ...
  program_text = <<-EOF
    base_filtering = filter('namespace', 'AWS/ECS')
    signal = data('MemoryUtilization', filter=base_filtering and filter('stat', 'mean') and not filter('ServiceName', '*') and ${module.filtering.signalflow})${var.memory_utilization_aggregation_function}${var.memory_utilization_transformation_function}.publish('signal')
