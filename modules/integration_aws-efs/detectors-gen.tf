resource "signalfx_detector" "used_space" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.used_space_threshold_critical}, lasting=${var.used_space_lasting_duration_critical == null ? "None" : format("'%s'", var.used_space_lasting_duration_critical)}, at_least=${var.used_space_at_least_percentage_critical})).publish('CRIT')
    detect(when(signal > ${var.used_space_threshold_major}, lasting=${var.used_space_lasting_duration_major == null ? "None" : format("'%s'", var.used_space_lasting_duration_major)}, at_least=${var.used_space_at_least_percentage_major}) and (not when(signal > ${var.used_space_threshold_critical}, lasting=${var.used_space_lasting_duration_critical == null ? "None" : format("'%s'", var.used_space_lasting_duration_critical)}, at_least=${var.used_space_at_least_percentage_critical}))).publish('MAJOR')
EOF
...
resource "signalfx_detector" "io_limit" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.io_limit_threshold_major}, lasting=${var.io_limit_lasting_duration_major == null ? "None" : format("'%s'", var.io_limit_lasting_duration_major)}, at_least=${var.io_limit_at_least_percentage_major})).publish('MAJOR')
    detect(when(signal > ${var.io_limit_threshold_minor}, lasting=${var.io_limit_lasting_duration_minor == null ? "None" : format("'%s'", var.io_limit_lasting_duration_minor)}, at_least=${var.io_limit_at_least_percentage_minor}) and (not when(signal > ${var.io_limit_threshold_major}, lasting=${var.io_limit_lasting_duration_major == null ? "None" : format("'%s'", var.io_limit_lasting_duration_major)}, at_least=${var.io_limit_at_least_percentage_major}))).publish('MINOR')
EOF
...
resource "signalfx_detector" "read_throughput" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.read_throughput_threshold_minor}, lasting=${var.read_throughput_lasting_duration_minor == null ? "None" : format("'%s'", var.read_throughput_lasting_duration_minor)}, at_least=${var.read_throughput_at_least_percentage_minor})).publish('MINOR')
    detect(when(signal > ${var.read_throughput_threshold_warning}, lasting=${var.read_throughput_lasting_duration_warning == null ? "None" : format("'%s'", var.read_throughput_lasting_duration_warning)}, at_least=${var.read_throughput_at_least_percentage_warning}) and (not when(signal > ${var.read_throughput_threshold_minor}, lasting=${var.read_throughput_lasting_duration_minor == null ? "None" : format("'%s'", var.read_throughput_lasting_duration_minor)}, at_least=${var.read_throughput_at_least_percentage_minor}))).publish('WARN')
EOF
...
resource "signalfx_detector" "write_throughput" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.write_throughput_threshold_minor}, lasting=${var.write_throughput_lasting_duration_minor == null ? "None" : format("'%s'", var.write_throughput_lasting_duration_minor)}, at_least=${var.write_throughput_at_least_percentage_minor})).publish('MINOR')
    detect(when(signal > ${var.write_throughput_threshold_warning}, lasting=${var.write_throughput_lasting_duration_warning == null ? "None" : format("'%s'", var.write_throughput_lasting_duration_warning)}, at_least=${var.write_throughput_at_least_percentage_warning}) and (not when(signal > ${var.write_throughput_threshold_minor}, lasting=${var.write_throughput_lasting_duration_minor == null ? "None" : format("'%s'", var.write_throughput_lasting_duration_minor)}, at_least=${var.write_throughput_at_least_percentage_minor}))).publish('WARN')
EOF
...
resource "signalfx_detector" "percent_of_permitted_throughput" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.percent_of_permitted_throughput_threshold_major}, lasting=${var.percent_of_permitted_throughput_lasting_duration_major == null ? "None" : format("'%s'", var.percent_of_permitted_throughput_lasting_duration_major)}, at_least=${var.percent_of_permitted_throughput_at_least_percentage_major})).publish('MAJOR')
    detect(when(signal > ${var.percent_of_permitted_throughput_threshold_minor}, lasting=${var.percent_of_permitted_throughput_lasting_duration_minor == null ? "None" : format("'%s'", var.percent_of_permitted_throughput_lasting_duration_minor)}, at_least=${var.percent_of_permitted_throughput_at_least_percentage_minor}) and (not when(signal > ${var.percent_of_permitted_throughput_threshold_major}, lasting=${var.percent_of_permitted_throughput_lasting_duration_major == null ? "None" : format("'%s'", var.percent_of_permitted_throughput_lasting_duration_major)}, at_least=${var.percent_of_permitted_throughput_at_least_percentage_major}))).publish('MINOR')
EOF
...
resource "signalfx_detector" "burst_credit_balance" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal < ${var.burst_credit_balance_threshold_major}, lasting=${var.burst_credit_balance_lasting_duration_major == null ? "None" : format("'%s'", var.burst_credit_balance_lasting_duration_major)}, at_least=${var.burst_credit_balance_at_least_percentage_major})).publish('MAJOR')
EOF
