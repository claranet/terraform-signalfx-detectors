...
resource "signalfx_detector" "snat_port_utilization" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.snat_port_utilization_threshold_critical}, lasting=${var.snat_port_utilization_lasting_duration_critical == null ? "None" : format("'%s'", var.snat_port_utilization_lasting_duration_critical)}, at_least=${var.snat_port_utilization_at_least_percentage_critical})).publish('CRIT')
    detect(when(signal > ${var.snat_port_utilization_threshold_major}, lasting=${var.snat_port_utilization_lasting_duration_major == null ? "None" : format("'%s'", var.snat_port_utilization_lasting_duration_major)}, at_least=${var.snat_port_utilization_at_least_percentage_major}) and (not when(signal > ${var.snat_port_utilization_threshold_critical}, lasting=${var.snat_port_utilization_lasting_duration_critical == null ? "None" : format("'%s'", var.snat_port_utilization_lasting_duration_critical)}, at_least=${var.snat_port_utilization_at_least_percentage_critical}))).publish('MAJOR')
EOF
...
resource "signalfx_detector" "throughput" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal >= ${var.throughput_threshold_critical}, lasting=${var.throughput_lasting_duration_critical == null ? "None" : format("'%s'", var.throughput_lasting_duration_critical)}, at_least=${var.throughput_at_least_percentage_critical})).publish('CRIT')
    detect(when(signal >= ${var.throughput_threshold_major}, lasting=${var.throughput_lasting_duration_major == null ? "None" : format("'%s'", var.throughput_lasting_duration_major)}, at_least=${var.throughput_at_least_percentage_major}) and (not when(signal >= ${var.throughput_threshold_critical}, lasting=${var.throughput_lasting_duration_critical == null ? "None" : format("'%s'", var.throughput_lasting_duration_critical)}, at_least=${var.throughput_at_least_percentage_critical}))).publish('MAJOR')
    detect(when(signal >= ${var.throughput_threshold_minor}, lasting=${var.throughput_lasting_duration_minor == null ? "None" : format("'%s'", var.throughput_lasting_duration_minor)}, at_least=${var.throughput_at_least_percentage_minor}) and (not when(signal >= ${var.throughput_threshold_major}, lasting=${var.throughput_lasting_duration_major == null ? "None" : format("'%s'", var.throughput_lasting_duration_major)}, at_least=${var.throughput_at_least_percentage_major}))).publish('MINOR')
    detect(when(signal >= ${var.throughput_threshold_warning}, lasting=${var.throughput_lasting_duration_warning == null ? "None" : format("'%s'", var.throughput_lasting_duration_warning)}, at_least=${var.throughput_at_least_percentage_warning}) and (not when(signal >= ${var.throughput_threshold_warning}, lasting=${var.throughput_lasting_duration_warning == null ? "None" : format("'%s'", var.throughput_lasting_duration_warning)}, at_least=${var.throughput_at_least_percentage_warning}))).publish('WARN')
EOF
...
resource "signalfx_detector" "health_state" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal < ${var.health_state_threshold_critical}, lasting=${var.health_state_lasting_duration_critical == null ? "None" : format("'%s'", var.health_state_lasting_duration_critical)}, at_least=${var.health_state_at_least_percentage_critical})).publish('CRIT')
    detect(when(signal < ${var.health_state_threshold_major}, lasting=${var.health_state_lasting_duration_major == null ? "None" : format("'%s'", var.health_state_lasting_duration_major)}, at_least=${var.health_state_at_least_percentage_major}) and (not when(signal < ${var.health_state_threshold_critical}, lasting=${var.health_state_lasting_duration_critical == null ? "None" : format("'%s'", var.health_state_lasting_duration_critical)}, at_least=${var.health_state_at_least_percentage_critical}))).publish('MAJOR')
EOF
