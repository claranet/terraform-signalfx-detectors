...
resource "signalfx_detector" "snat_port_utilization" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.snat_port_utilization_threshold_critical}%{if var.snat_port_utilization_lasting_duration_critical != null}, lasting='${var.snat_port_utilization_lasting_duration_critical}', at_least=${var.snat_port_utilization_at_least_percentage_critical}%{endif})).publish('CRIT')
    detect(when(signal > ${var.snat_port_utilization_threshold_major}%{if var.snat_port_utilization_lasting_duration_major != null}, lasting='${var.snat_port_utilization_lasting_duration_major}', at_least=${var.snat_port_utilization_at_least_percentage_major}%{endif}) and (not when(signal > ${var.snat_port_utilization_threshold_critical}%{if var.snat_port_utilization_lasting_duration_critical != null}, lasting='${var.snat_port_utilization_lasting_duration_critical}', at_least=${var.snat_port_utilization_at_least_percentage_critical}%{endif}))).publish('MAJOR')
EOF
...
resource "signalfx_detector" "throughput" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal >= ${var.throughput_threshold_critical}%{if var.throughput_lasting_duration_critical != null}, lasting='${var.throughput_lasting_duration_critical}', at_least=${var.throughput_at_least_percentage_critical}%{endif})).publish('CRIT')
    detect(when(signal >= ${var.throughput_threshold_major}%{if var.throughput_lasting_duration_major != null}, lasting='${var.throughput_lasting_duration_major}', at_least=${var.throughput_at_least_percentage_major}%{endif}) and (not when(signal >= ${var.throughput_threshold_critical}%{if var.throughput_lasting_duration_critical != null}, lasting='${var.throughput_lasting_duration_critical}', at_least=${var.throughput_at_least_percentage_critical}%{endif}))).publish('MAJOR')
    detect(when(signal >= ${var.throughput_threshold_minor}%{if var.throughput_lasting_duration_minor != null}, lasting='${var.throughput_lasting_duration_minor}', at_least=${var.throughput_at_least_percentage_minor}%{endif}) and (not when(signal >= ${var.throughput_threshold_major}%{if var.throughput_lasting_duration_major != null}, lasting='${var.throughput_lasting_duration_major}', at_least=${var.throughput_at_least_percentage_major}%{endif}))).publish('MINOR')
    detect(when(signal >= ${var.throughput_threshold_warning}%{if var.throughput_lasting_duration_warning != null}, lasting='${var.throughput_lasting_duration_warning}', at_least=${var.throughput_at_least_percentage_warning}%{endif}) and (not when(signal >= ${var.throughput_threshold_warning}%{if var.throughput_lasting_duration_warning != null}, lasting='${var.throughput_lasting_duration_warning}', at_least=${var.throughput_at_least_percentage_warning}%{endif}))).publish('WARN')
EOF
...
resource "signalfx_detector" "health_state" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal < ${var.health_state_threshold_critical}%{if var.health_state_lasting_duration_critical != null}, lasting='${var.health_state_lasting_duration_critical}', at_least=${var.health_state_at_least_percentage_critical}%{endif})).publish('CRIT')
    detect(when(signal < ${var.health_state_threshold_major}%{if var.health_state_lasting_duration_major != null}, lasting='${var.health_state_lasting_duration_major}', at_least=${var.health_state_at_least_percentage_major}%{endif}) and (not when(signal < ${var.health_state_threshold_critical}%{if var.health_state_lasting_duration_critical != null}, lasting='${var.health_state_lasting_duration_critical}', at_least=${var.health_state_at_least_percentage_critical}%{endif}))).publish('MAJOR')
EOF
