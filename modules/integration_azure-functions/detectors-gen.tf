resource "signalfx_detector" "errors" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.errors_threshold_critical}, lasting=${var.errors_lasting_duration_critical == null ? "None" : format("'%s'", var.errors_lasting_duration_critical)}, at_least=${var.errors_at_least_percentage_critical})).publish('CRIT')
    detect(when(signal > ${var.errors_threshold_major}, lasting=${var.errors_lasting_duration_major == null ? "None" : format("'%s'", var.errors_lasting_duration_major)}, at_least=${var.errors_at_least_percentage_major}) and (not when(signal > ${var.errors_threshold_critical}, lasting=${var.errors_lasting_duration_critical == null ? "None" : format("'%s'", var.errors_lasting_duration_critical)}, at_least=${var.errors_at_least_percentage_critical}))).publish('MAJOR')
EOF
