resource "signalfx_detector" "errors" {
  ...
  program_text = <<-EOF
    ...
    errors = data('azure.function.errors', filter=base_filtering and ${module.filter-tags.filter_custom}, rollup='sum', extrapolation='last_value')${var.errors_aggregation_function}${var.errors_transformation_function}
    invocations = data('azure.function.invocations', filter=base_filtering and ${module.filter-tags.filter_custom}, rollup='sum', extrapolation='last_value')${var.errors_aggregation_function}${var.errors_transformation_function}
    ...
    detect(when(signal > ${var.errors_threshold_major}, lasting='${var.errors_lasting_duration_major}') and when(signal <= ${var.errors_threshold_critical})).publish('MAJOR')
EOF
