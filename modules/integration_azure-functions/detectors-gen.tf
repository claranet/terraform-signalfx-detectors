resource "signalfx_detector" "errors" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.errors_threshold_critical}%{if var.errors_lasting_duration_critical != "None"}, lasting='${var.errors_lasting_duration_critical}', at_least=${var.errors_at_least_percentage_critical}%{endif})).publish('CRIT')
    detect(when(signal > ${var.errors_threshold_major}%{if var.errors_lasting_duration_major != "None"}, lasting='${var.errors_lasting_duration_major}', at_least=${var.errors_at_least_percentage_major}%{endif}) and not when(signal > ${var.errors_threshold_critical}%{if var.errors_lasting_duration_critical != "None"}, lasting='${var.errors_lasting_duration_critical}', at_least=${var.errors_at_least_percentage_critical}%{endif})).publish('MAJOR')
EOF
