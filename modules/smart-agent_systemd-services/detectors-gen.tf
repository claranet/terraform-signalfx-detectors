resource "signalfx_detector" "aliveness" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal != ${var.aliveness_threshold_critical}%{if var.aliveness_lasting_duration_critical != null}, lasting='${var.aliveness_lasting_duration_critical}', at_least=${var.aliveness_at_least_percentage_critical}%{endif})).publish('CRIT')
EOF
