resource "signalfx_detector" "file_backup" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal < ${var.file_backup_threshold_critical}%{if var.file_backup_lasting_duration_critical != null}, lasting='${var.file_backup_lasting_duration_critical}', at_least=${var.file_backup_at_least_percentage_critical}%{endif})).publish('CRIT')
EOF
