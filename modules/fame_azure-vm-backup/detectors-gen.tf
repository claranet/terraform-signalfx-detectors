resource "signalfx_detector" "backup" {
  ...
  program_text = <<-EOF
    signal = data('fame.azure.backup.vm', filter=${module.filtering.signalflow}, extrapolation='zero')${var.backup_aggregation_function}${var.backup_transformation_function}.publish('signal')
