resource "signalfx_detector" "error_rate_5xx" {
  ...
  program_text = <<-EOF
    errors = data('https/request_count', filter=filter('service', 'loadbalancing') and filter('response_code_class', '500') and ${module.filtering.signalflow}, rollup='sum', extrapolation='zero')${var.error_rate_5xx_aggregation_function}${var.error_rate_5xx_transformation_function}
    requests = data('https/request_count', filter=filter('service', 'loadbalancing') and ${module.filtering.signalflow}, rollup='sum', extrapolation='zero')${var.error_rate_5xx_aggregation_function}${var.error_rate_5xx_transformation_function}
...
resource "signalfx_detector" "error_rate_4xx" {
  ...
  program_text = <<-EOF
    errors = data('https/request_count', filter=filter('service', 'loadbalancing') and filter('response_code_class', '400') and ${module.filtering.signalflow}, rollup='sum', extrapolation='zero')${var.error_rate_4xx_aggregation_function}${var.error_rate_4xx_transformation_function}
    requests = data('https/request_count', filter=filter('service', 'loadbalancing') and ${module.filtering.signalflow}, rollup='sum', extrapolation='zero')${var.error_rate_4xx_aggregation_function}${var.error_rate_4xx_transformation_function}
    signal = (errors/requests).scale(100).fill(value=0).publish('signal')
...
resource "signalfx_detector" "backend_latency_service" {
  ...
  program_text = <<-EOF
    signal = data('https/backend_latencies', filter=filter('service', 'loadbalancing') and filter('backend_target_type', 'BACKEND_SERVICE') and ${module.filtering.signalflow}, rollup='average', extrapolation='zero')${var.backend_latency_service_aggregation_function}${var.backend_latency_service_transformation_function}.publish('signal')
...
resource "signalfx_detector" "backend_latency_bucket" {
  ...
  program_text = <<-EOF
    signal = data('https/backend_latencies', filter=filter('service', 'loadbalancing') and filter('backend_target_type', 'BACKEND_BUCKET') and ${module.filtering.signalflow}, rollup='average', extrapolation='zero')${var.backend_latency_bucket_aggregation_function}${var.backend_latency_bucket_transformation_function}.publish('signal')
...
resource "signalfx_detector" "request_count" {
  ...
  program_text = <<-EOF
    signal = data('https/request_count', filter=filter('service', 'loadbalancing') and ${module.filtering.signalflow}, rollup='sum', extrapolation='last_value')${var.request_count_aggregation_function}${var.request_count_transformation_function}.publish('signal')
