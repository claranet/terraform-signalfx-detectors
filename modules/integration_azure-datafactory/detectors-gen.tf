resource "signalfx_detector" "activity_error_rate" {
  ...
  program_text = <<-EOF
    adf_activity_succeeded_run = data('ActivitySucceededRuns', filter=filter('resource_type', 'Microsoft.DataFactory/factories') and filter('primary_aggregation_type', 'true') and ${module.filtering.signalflow})${var.activity_error_rate_aggregation_function}${var.activity_error_rate_transformation_function}
    adf_activity_failed_run = data('ActivityFailedRuns', filter=filter('resource_type', 'Microsoft.DataFactory/factories') and filter('primary_aggregation_type', 'true') and ${module.filtering.signalflow})${var.activity_error_rate_aggregation_function}${var.activity_error_rate_transformation_function}
...
resource "signalfx_detector" "pipeline_error_rate" {
  ...
  program_text = <<-EOF
    adf_pipeline_succeeded_run = data('PipelineSucceededRuns', filter=filter('resource_type', 'Microsoft.DataFactory/factories') and filter('primary_aggregation_type', 'true') and ${module.filtering.signalflow})${var.pipeline_error_rate_aggregation_function}${var.pipeline_error_rate_transformation_function}
    adf_pipeline_failed_run = data('PipelineFailedRuns', filter=filter('resource_type', 'Microsoft.DataFactory/factories') and filter('primary_aggregation_type', 'true') and ${module.filtering.signalflow})${var.pipeline_error_rate_aggregation_function}${var.pipeline_error_rate_transformation_function}
...
resource "signalfx_detector" "trigger_error_rate" {
  ...
  program_text = <<-EOF
    adf_trigger_succeeded_run = data('triggerSucceededRuns', filter=filter('resource_type', 'Microsoft.DataFactory/factories') and filter('primary_aggregation_type', 'true') and ${module.filtering.signalflow})${var.trigger_error_rate_aggregation_function}${var.trigger_error_rate_transformation_function}
    adf_trigger_failed_run = data('triggerFailedRuns', filter=filter('resource_type', 'Microsoft.DataFactory/factories') and filter('primary_aggregation_type', 'true') and ${module.filtering.signalflow})${var.trigger_error_rate_aggregation_function}${var.trigger_error_rate_transformation_function}
...
resource "signalfx_detector" "available_memory" {
  ...
  program_text = <<-EOF
    memory = data('IntegrationRuntimeAvailableMemory', filter=filter('resource_type', 'Microsoft.DataFactory/factories') and filter('primary_aggregation_type', 'true') and ${module.filtering.signalflow})${var.available_memory_aggregation_function}${var.available_memory_transformation_function}
...
resource "signalfx_detector" "cpu_percentage" {
  ...
  program_text = <<-EOF
    signal = data('IntegrationRuntimeCpuPercentage', filter=filter('resource_type', 'Microsoft.DataFactory/factories') and filter('primary_aggregation_type', 'true') and ${module.filtering.signalflow})${var.cpu_percentage_aggregation_function}${var.cpu_percentage_transformation_function}.publish('signal')
