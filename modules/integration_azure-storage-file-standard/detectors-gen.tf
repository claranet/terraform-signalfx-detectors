resource "signalfx_detector" "capacity" {
  ...
  program_text = <<-EOF
    base_filtering = filter('resource_type', 'Microsoft.Storage/storageAccounts/fileServices') and filter('primary_aggregation_type', 'true') and not filter('azure_sa_sku', ' Premium_*')
    ...
    signal = (capacity.scale(0.0000000000009094947017729282) / 100).scale(100).publish('signal')
...
resource "signalfx_detector" "iops" {
  ...
  program_text = <<-EOF
    ...
    signal = (transactions / 20000).scale(100).publish('signal')
...
resource "signalfx_detector" "throughput" {
  ...
  program_text = <<-EOF
    base_filtering = filter('resource_type', 'Microsoft.Storage/storageAccounts/fileServices') and filter('primary_aggregation_type', 'true') and not filter('azure_sa_sku', ' Premium_*')
    ingress = data('Ingress', filter=base_filtering and ${module.filtering.signalflow}, rollup='rate')${var.throughput_aggregation_function}${var.throughput_transformation_function}
    egress = data('Egress', filter=base_filtering and ${module.filtering.signalflow}, rollup='rate')${var.throughput_aggregation_function}${var.throughput_transformation_function}
    signal = ((ingress + egress).scale(0.00000095367431640625) / 300).scale(100).publish('signal')
