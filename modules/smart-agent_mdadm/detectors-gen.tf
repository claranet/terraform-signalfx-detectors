resource "signalfx_detector" "disk_failed" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.disk_failed_threshold_major}) and not when(signal > ${var.disk_failed_threshold_critical})).publish('MAJOR')
EOF
...
resource "signalfx_detector" "disk_missing" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.disk_missing_threshold_major}) and not when(signal > ${var.disk_missing_threshold_critical})).publish('MAJOR')
EOF
