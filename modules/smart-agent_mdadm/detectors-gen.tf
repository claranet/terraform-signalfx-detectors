resource "signalfx_detector" "disk_failed" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.disk_failed_threshold_critical}, lasting=${var.disk_failed_lasting_duration_critical == null ? "None" : format("'%s'", var.disk_failed_lasting_duration_critical)}, at_least=${var.disk_failed_at_least_percentage_critical})).publish('CRIT')
    detect(when(signal > ${var.disk_failed_threshold_major}, lasting=${var.disk_failed_lasting_duration_major == null ? "None" : format("'%s'", var.disk_failed_lasting_duration_major)}, at_least=${var.disk_failed_at_least_percentage_major}) and (not when(signal > ${var.disk_failed_threshold_critical}, lasting=${var.disk_failed_lasting_duration_critical == null ? "None" : format("'%s'", var.disk_failed_lasting_duration_critical)}, at_least=${var.disk_failed_at_least_percentage_critical}))).publish('MAJOR')
EOF
...
resource "signalfx_detector" "disk_missing" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.disk_missing_threshold_critical}, lasting=${var.disk_missing_lasting_duration_critical == null ? "None" : format("'%s'", var.disk_missing_lasting_duration_critical)}, at_least=${var.disk_missing_at_least_percentage_critical})).publish('CRIT')
    detect(when(signal > ${var.disk_missing_threshold_major}, lasting=${var.disk_missing_lasting_duration_major == null ? "None" : format("'%s'", var.disk_missing_lasting_duration_major)}, at_least=${var.disk_missing_at_least_percentage_major}) and (not when(signal > ${var.disk_missing_threshold_critical}, lasting=${var.disk_missing_lasting_duration_critical == null ? "None" : format("'%s'", var.disk_missing_lasting_duration_critical)}, at_least=${var.disk_missing_at_least_percentage_critical}))).publish('MAJOR')
EOF
