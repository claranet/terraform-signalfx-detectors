resource "signalfx_detector" "disk_failed" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.disk_failed_threshold_critical}%{if var.disk_failed_lasting_duration_critical != null}, lasting='${var.disk_failed_lasting_duration_critical}', at_least=${var.disk_failed_at_least_percentage_critical}%{endif})).publish('CRIT')
    detect(when(signal > ${var.disk_failed_threshold_major}%{if var.disk_failed_lasting_duration_major != null}, lasting='${var.disk_failed_lasting_duration_major}', at_least=${var.disk_failed_at_least_percentage_major}%{endif}) and (not when(signal > ${var.disk_failed_threshold_critical}%{if var.disk_failed_lasting_duration_critical != null}, lasting='${var.disk_failed_lasting_duration_critical}', at_least=${var.disk_failed_at_least_percentage_critical}%{endif}))).publish('MAJOR')
EOF
...
resource "signalfx_detector" "disk_missing" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.disk_missing_threshold_critical}%{if var.disk_missing_lasting_duration_critical != null}, lasting='${var.disk_missing_lasting_duration_critical}', at_least=${var.disk_missing_at_least_percentage_critical}%{endif})).publish('CRIT')
    detect(when(signal > ${var.disk_missing_threshold_major}%{if var.disk_missing_lasting_duration_major != null}, lasting='${var.disk_missing_lasting_duration_major}', at_least=${var.disk_missing_at_least_percentage_major}%{endif}) and (not when(signal > ${var.disk_missing_threshold_critical}%{if var.disk_missing_lasting_duration_critical != null}, lasting='${var.disk_missing_lasting_duration_critical}', at_least=${var.disk_missing_at_least_percentage_critical}%{endif}))).publish('MAJOR')
EOF
