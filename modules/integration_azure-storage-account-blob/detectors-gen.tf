resource "signalfx_detector" "requests_error_rate" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.requests_error_rate_threshold_critical}, lasting=${var.requests_error_rate_lasting_duration_critical == null ? "None" : format("'%s'", var.requests_error_rate_lasting_duration_critical)}, at_least=${var.requests_error_rate_at_least_percentage_critical})).publish('CRIT')
    detect(when(signal > ${var.requests_error_rate_threshold_major}, lasting=${var.requests_error_rate_lasting_duration_major == null ? "None" : format("'%s'", var.requests_error_rate_lasting_duration_major)}, at_least=${var.requests_error_rate_at_least_percentage_major}) and (not when(signal > ${var.requests_error_rate_threshold_critical}, lasting=${var.requests_error_rate_lasting_duration_critical == null ? "None" : format("'%s'", var.requests_error_rate_lasting_duration_critical)}, at_least=${var.requests_error_rate_at_least_percentage_critical}))).publish('MAJOR')
EOF
...
resource "signalfx_detector" "latency_e2e" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.latency_e2e_threshold_critical}, lasting=${var.latency_e2e_lasting_duration_critical == null ? "None" : format("'%s'", var.latency_e2e_lasting_duration_critical)}, at_least=${var.latency_e2e_at_least_percentage_critical})).publish('CRIT')
    detect(when(signal > ${var.latency_e2e_threshold_major}, lasting=${var.latency_e2e_lasting_duration_major == null ? "None" : format("'%s'", var.latency_e2e_lasting_duration_major)}, at_least=${var.latency_e2e_at_least_percentage_major}) and (not when(signal > ${var.latency_e2e_threshold_critical}, lasting=${var.latency_e2e_lasting_duration_critical == null ? "None" : format("'%s'", var.latency_e2e_lasting_duration_critical)}, at_least=${var.latency_e2e_at_least_percentage_critical}))).publish('MAJOR')
EOF
