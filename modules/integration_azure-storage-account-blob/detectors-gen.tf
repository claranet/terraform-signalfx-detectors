resource "signalfx_detector" "requests_error_rate" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.requests_error_rate_threshold_critical}%{if var.requests_error_rate_lasting_duration_critical != null}, lasting='${var.requests_error_rate_lasting_duration_critical}', at_least=${var.requests_error_rate_at_least_percentage_critical}%{endif})).publish('CRIT')
    detect(when(signal > ${var.requests_error_rate_threshold_major}%{if var.requests_error_rate_lasting_duration_major != null}, lasting='${var.requests_error_rate_lasting_duration_major}', at_least=${var.requests_error_rate_at_least_percentage_major}%{endif}) and (not when(signal > ${var.requests_error_rate_threshold_critical}%{if var.requests_error_rate_lasting_duration_critical != null}, lasting='${var.requests_error_rate_lasting_duration_critical}', at_least=${var.requests_error_rate_at_least_percentage_critical}%{endif}))).publish('MAJOR')
EOF
...
resource "signalfx_detector" "latency_e2e" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.latency_e2e_threshold_critical}%{if var.latency_e2e_lasting_duration_critical != null}, lasting='${var.latency_e2e_lasting_duration_critical}', at_least=${var.latency_e2e_at_least_percentage_critical}%{endif})).publish('CRIT')
    detect(when(signal > ${var.latency_e2e_threshold_major}%{if var.latency_e2e_lasting_duration_major != null}, lasting='${var.latency_e2e_lasting_duration_major}', at_least=${var.latency_e2e_at_least_percentage_major}%{endif}) and (not when(signal > ${var.latency_e2e_threshold_critical}%{if var.latency_e2e_lasting_duration_critical != null}, lasting='${var.latency_e2e_lasting_duration_critical}', at_least=${var.latency_e2e_at_least_percentage_critical}%{endif}))).publish('MAJOR')
EOF
