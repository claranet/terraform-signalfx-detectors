resource "signalfx_detector" "requests_error_rate" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.requests_error_rate_threshold_major} and signal <= ${var.requests_error_rate_threshold_critical})).publish('MAJOR')
EOF
...
resource "signalfx_detector" "latency_e2e" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.latency_e2e_threshold_major} and signal <= ${var.latency_e2e_threshold_critical})).publish('MAJOR')
EOF
