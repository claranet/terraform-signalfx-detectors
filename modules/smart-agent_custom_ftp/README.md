...
```hcl
module "signalfx-detectors-smart-agent-ftp" {
  source = "github.com/claranet/terraform-signalfx-detectors.git//modules/smart-agent_custom_ftp?ref={revision}"
...
(webchecks often require accuracy). I.e. `max(over='15m')` on `ftp_code_matched` will
