...
resource "signalfx_detector" "deadlettered_messages" {
  ...
  program_text = <<-EOF
        ...
        detect(when(signal > threshold(${var.deadlettered_messages_threshold_major}), lasting="${var.deadlettered_messages_timer}") and when(signal <= ${var.deadlettered_messages_threshold_critical})).publish('MAJOR')).publish('MAJOR')
    EOF
