...
resource "signalfx_detector" "cpu_usage" {
  name = format("%s %s", local.detector_name_prefix, "AWS RDS Common cpu usage")
...
resource "signalfx_detector" "free_space" {
  name = format("%s %s", local.detector_name_prefix, "AWS RDS Common free space")
  ...
  program_text = <<-EOF
    base_filtering = filter('namespace', 'AWS/RDS')
    signal = data('FreeStorageSpace', filter=base_filtering and filter('stat', 'mean') and filter('DBInstanceIdentifier', '*') and ${module.filtering.signalflow})${var.free_space_aggregation_function}${var.free_space_transformation_function}.publish('signal')
