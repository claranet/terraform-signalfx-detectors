...
resource "signalfx_detector" "replication_lag" {
  ...
  program_text = <<-EOF
        ...
        detect(when(signal > threshold(${var.replication_lag_threshold_critical}), lasting="${var.replication_lag_lasting_duration_critical}")).publish('CRIT')
        detect(when(signal > threshold(${var.replication_lag_threshold_major}), lasting="${var.replication_lag_lasting_duration_major}") and (not when(signal > ${var.replication_lag_threshold_critical}, lasting="${var.replication_lag_lasting_duration_critical}"))).publish('MAJOR')
    EOF
