...
resource "signalfx_detector" "deadlocks_count" {
  ...
  program_text = <<-EOF
    ...
    detect(when(signal > ${var.deadlocks_count_threshold_critical}, lasting=%{if var.deadlocks_count_lasting_duration_critical == null}None%{else}'${var.deadlocks_count_lasting_duration_critical}'%{endif}, at_least=${var.deadlocks_count_at_least_percentage_critical})).publish('CRIT')
EOF
