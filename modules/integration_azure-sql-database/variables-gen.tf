...
variable "deadlocks_count_transformation_function" {
  description = "Transformation function for deadlocks_count detector (i.e. \".mean(over='5m')\")"
  type        = string
  default     = ""
}
...
variable "deadlocks_count_runbook_url" {
  description = "URL like SignalFx dashboard or wiki page which can help to troubleshoot the incident cause"
  type        = string
  default     = ""
}
...
variable "deadlocks_count_lasting_duration_critical" {
  description = "Minimum duration that conditions must be true before raising alert"
  type        = string
  default     = "5m"
}

variable "deadlocks_count_at_least_percentage_critical" {
  description = "Percentage of lasting that conditions must be true before raising alert (>= 0.0 and <= 1.0)"
  type        = number
  default     = 1
}
